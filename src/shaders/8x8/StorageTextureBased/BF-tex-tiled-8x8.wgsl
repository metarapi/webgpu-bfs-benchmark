...
// Shared tiles
var<workgroup> wgTerrainA : array<f32, T>;
var<workgroup> wgDistA    : array<f32, T>;
...
fn loadTile(
    ...
    loop {
        ...
        wgDistA[ti]    = dist;
        wgDistB[ti]    = dist; // keep B halo consistent before ping-pong
...
fn relax_one_parity(
    ...
    for (var n: u32 = 0u; n < 8u; n = n + 1u) {
        ...
        if (nDist > 0.0 && nTerrain != 0.0) {
            // Diagonal: enforce corner-cut rule
...
fn relax_tile_fixed_no_early(r: u32, local_id: vec3<u32>) -> u32 {
    ...
    return 1u - parity;              // last writes are in opposite of next parity
}
...
fn storeTile(
    ...
    loop {
        ...
        let valA = wgDistA[ti];
        let valB = wgDistB[ti];
        let val  = select(valA, valB, finalParity == 0u);
...
fn main(
    ...
    // 1) Load tile into shared memory from storage textures
    loadTile(tile_origin_x, tile_origin_y, lidx);

    // 2) r inner sweeps in shared memory
    let r : u32 = uniforms.iterations;
    let finalParity = relax_tile_fixed(r, lid);

    // 3) Write-back interior from shared memory to distTexOut
